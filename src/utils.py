...
def visualize_comparison(df_true, df_filtered, time_col=None):
    """
    Порівняльна візуалізація оригінальних (зашумлених) і очищених даних.

    Аргументи:
      df_true     – DataFrame із сирими, зашумленими даними.
      df_filtered – DataFrame із очищеними значеннями після обробки.
      time_col    – (опціонально) назва стовпця з часовими мітками (якщо є). Якщо не задано, використовується індекс.
    """
    # Визначаємо часову шкалу
    if time_col is None:
        time = df_true.index
    else:
        time = df_true[time_col]

    # -------------------------------
    # 1. Часові ряди (Оригінал vs Очищені)
    # -------------------------------
    num_plots = 2
    if 'solid_feed_percent' in df_true.columns:
        num_plots = 3
    ...
    axes[0].plot(time, df_true['feed_fe_percent'], color='tab:blue', linestyle='--', alpha=0.6, label='Оригінальні')
    axes[0].plot(time, df_filtered['feed_fe_percent'], color='tab:blue', label='Очищені')
    axes[0].set_title('feed_fe_percent: Оригінальні vs Очищені')
    axes[0].set_ylabel('Feed Fe %')
    axes[0].legend()
    axes[0].grid(True)

    axes[1].plot(time, df_true['ore_mass_flow'], color='tab:orange', linestyle='--', alpha=0.6, label='Оригінальні')
    axes[1].plot(time, df_filtered['ore_mass_flow'], color='tab:orange', label='Очищені')
    axes[1].set_title('ore_mass_flow: Оригінальні vs Очищені')
    axes[1].set_ylabel('Ore Mass Flow')
    axes[1].legend()
    axes[1].grid(True)

    if num_plots == 3:
        axes[2].plot(time, df_true['solid_feed_percent'], color='tab:green', linestyle='--', alpha=0.6, label='Оригінальні')
        axes[2].plot(time, df_filtered['solid_feed_percent'], color='tab:green', label='Очищені')
        axes[2].set_title('solid_feed_percent: Оригінальні vs Очищені')
        axes[2].set_ylabel('Solid Feed %')
    ...
    # -------------------------------
    # 2. Гістограми розподілу (Оригінал vs Очищені)
    # -------------------------------
    num_hist = 2
    if 'solid_feed_percent' in df_true.columns:
        num_hist = 3

    fig, axes = plt.subplots(1, num_hist, figsize=(6*num_hist, 5))
    if num_hist == 1:
        axes = [axes]

    axes[0].hist(df_true['feed_fe_percent'], bins=30, alpha=0.5, color='tab:blue', label='Оригінальні')
    axes[0].hist(df_filtered['feed_fe_percent'], bins=30, alpha=0.5, color='tab:cyan', label='Очищені')
    axes[0].set_title('Гістограма: feed_fe_percent')
    axes[0].set_xlabel('Feed Fe %')
    axes[0].set_ylabel('Частота')
    axes[0].legend()
    axes[0].grid(True)

    axes[1].hist(df_true['ore_mass_flow'], bins=30, alpha=0.5, color='tab:orange', label='Оригінальні')
    axes[1].hist(df_filtered['ore_mass_flow'], bins=30, alpha=0.5, color='tab:red', label='Очищені')
    axes[1].set_title('Гістограма: ore_mass_flow')
    axes[1].set_xlabel('Ore Mass Flow')
    axes[1].set_ylabel('Частота')
    axes[1].legend()
    axes[1].grid(True)

    if num_hist == 3:
        axes[2].hist(df_true['solid_feed_percent'], bins=30, alpha=0.5, color='tab:green', label='Оригінальні')
        axes[2].hist(df_filtered['solid_feed_percent'], bins=30, alpha=0.5, color='lime', label='Очищені')
        axes[2].set_title('Гістограма: solid_feed_percent')
        axes[2].set_xlabel('Solid Feed %')
...
# Виклик функції
# visualize_comparison(df_true, df_filtered)  # Якщо часові мітки у індексі
# або
# visualize_comparison(df_true, df_filtered, time_col='time')  # Якщо є стовпець 'time'